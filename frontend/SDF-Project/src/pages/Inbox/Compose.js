...
function Compose({sendTo}) {
  const {adminID}=useParams()
    ...
    const handleSend=async()=>{
      const res=await axios.post(`http://192.168.51.89:5001/messages/admin/${adminID}`,{
        ...
        <div className="page-content-compose">
            ...
            <div className="btn-for-send">
              <div className="action-sent">
               <Link to={`/${adminID}/inbox`}><button className="btn-primary btn mx-3 px-3">Inbox</button></Link> 
               <Link to={`/${adminID}/inbox/sent`}> <button className="btn-primary btn mx-3 px-3">Sent Items</button></Link>
